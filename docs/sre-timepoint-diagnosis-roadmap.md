...
- 将通用时间点诊断升级为场景诊断包，第一版提供 CI/CD 发布诊断模板。
- 没有日志源或日志源不可用时，诊断任务仍可基于指标、告警和 AnsFlow 内部上下文生成降级报告。
- 用户新增常规日志源时尽量不改后端代码，只通过数据源类型、查询模板、字段映射和响应映射完成接入。

## 诊断模板

诊断模板用于把“诊断目标、上下文采集、日志关键词、AI Prompt 和结构化报告要求”固化为可复用的场景包。模板分为全局模板和项目模板；项目模板与全局模板使用相同 `code` 时，项目模板优先。运行诊断时会把模板快照保存到 `DiagnosisRun.query_params.template_snapshot`，保证后续模板修改不会影响历史诊断解释。
...
1. 进入 `/v1/sre/diagnosis` 的“诊断模板”页签，查看内置模板或复制为项目模板。
2. 创建诊断时选择 CI/CD 模板，并填写流水线运行 ID、节点运行 ID 或 Ansible 执行 ID。
3. 诊断任务按模板采集上下文，输出诊断结论、证据引用和处置建议。

第一版只提供建议，不自动创建或执行修复流水线，也不执行 SSH、kubectl、Arthas 等实时采样命令。
